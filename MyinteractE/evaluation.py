...
def ranking_and_hits(model, test_data_batch, chequer_perm, perm, matrix_w, matrix_h, ker_size, device):
    ranks = []
    ranks_left = []
    ranks_right = []
    hits_one = []
    ...
    for index, data in enumerate(test_data_batch):
        ...
        pred1 = model.prodict(h, r, chequer_perm, perm, matrix_w, matrix_h, ker_size)
        pred2 = model.prodict(t, r_inv, chequer_perm, perm, matrix_w, matrix_h, ker_size)

        list1 = torch.arange(pred1.size()[0]).to(device)
        list2 = torch.arange(pred2.size()[0]).to(device)
